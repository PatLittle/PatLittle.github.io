---
title: "Update - orphanhost.github.io"
category: "Development & Systems"
---

In my last post I talked about the [https://Gisthost.github.io](https://gisthost.github.io/) tool that Simon Willison revived by forking a version from a project that was no longer being maintained. 

Flip back a post to see that Here:

>[https://patlittle.github.io/blog/gisthost/](https://patlittle.github.io/blog/gisthost/)

Since posting that a few days ago, I noticed Simon created another project in GitHub [orphanhost.github.io](https://github.com/orphanhost/orphanhost.github.io).
This tool allows you to serve a HTML page from a file in a Github Repo, on any branch, without using a static site generator such as Jekyll, Hugo, etc.

For example [a11y-rpt-test-output/test.html](https://orphanhost.github.io/?PatLittle/test/main/a11y-rpt-test-output/test.html) is an HTML file I have in a repo, but not being rendered as a static page anywhere.
Just adding the repository and file info is all you need to do to have it as an interactive page.

<img width="902" height="850" alt="image" src="https://github.com/user-attachments/assets/3ecd76ea-0e42-4606-b62e-01bf3821b2cf" />

A variation on GistHost.github.io that adds another option for you and your projects.

🆒🆒🆒

